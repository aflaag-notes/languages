...
\begin{document}
    ...
    \begin{frameddefn}{Polimorfismo}
        Con \tbf{polimorfismo} si definisce la possibilità, per un'espressione, di assumere molteplici tipi, in base al contesto --- inteso \tit{non} nel senso degli elementi in $\mathrm{Ctx}$ --- considerato. Le grammatiche polimorfe sono caratterizzate da \tbf{variabili di tipo}, che permettono di descrivere in maniera \tit{generica} le loro espressioni.
    \end{frameddefn}

    \begin{frameddefn}{Clausola dei tipi polimorfi}
        Sia $G$ una grammatica tipata polimorfa, dove $Types$ e $Terms$ costituiscono rispettivamente le grammatiche dei tipi e dei termini di $G$; allora, è possibile definire su $G$ una funzione $poly$, come segue: $$\func{poly}{Types \times Terms}{Terms}$$ e verrà utilizzata attraverso la sintassi $$\forall \ttt{:type:}_1.\ttt{:type:}_2$$ la quale asserisce che, per ogni tipo che la varibile di tipo $\ttt{:type:}_1$ può assumere, il tipo polimorfo definito è $\ttt{:type:}_2$.
    \end{frameddefn}
    ...
    \begin{frameddefn}{Lambda calcolo polimorfo}
        ...
        \begin{itemize}
            \item una grammatica per i \tit{tipi}, che verrà indicata con $Types$; si noti che il simbolismo $X$ indica che la grammatica include le variabili di tipo;
            \item una grammatica per i \tit{termini}, che verrà indicata con $Terms$.
        \end{itemize}
    \end{frameddefn}

    \begin{framedprop}{Regole di inferenza di \tit{System F}}
        ...
        \begin{itemize}
            \item \tbf{astrazioni di tipi}: \centeredeq{0.9}{$\forall M \in Terms, A \in Types \quad \exists X \in Types \mid X \notin \mathrm{free}(\Gamma) \implies [abstr] \ \dfrac{\typejud{\Gamma}{M}{A}}{\typejud{\Gamma}{\Lambda X.M}{\forall X.A}}$}
            \item \tbf{istanziazioni di tipi}: $$\forall M \in Terms, A, B, X \in Types \quad [inst] \ \dfrac{\typejud{\Gamma}{M}{\forall X.A}}{\typejud{\Gamma}{MB}{A[B/X]}}$$
        \end{itemize}
    \end{framedprop}

    \begin{example}[Tipo dell'dentità polimorfa]
