...
\begin{document}
    ...
    \begin{example}[Semantica operazionale di $Exp$]
        ...
        $$[const] \ \opjud{E}{k}{k}$$
        $$\forall x \in \mathrm{Var} \quad \exists v \in \mathrm{Val} \mid E(x) = v \implies [vars] \ \opjud{E}{x}{v}$$
    ...
    \begin{frameddefn}{Valutazione eager}
        Data una grammatica, la \tbf{valutazione eager} (\tit{call-by-name}) valuta una data espressione della grammatica non appena questa viene legata ad una variabile. In simboli, la valutazione eager verrà indicata con il pedice $\mathrm{E}$.
    \end{frameddefn}

    \begin{frameddefn}{Valutazione lazy}
        Data una grammatica, la \tbf{valutazione lazy} (\tit{call-by-need}) valuta una data espressione della grammatica solo quando il suo valore viene richiesto da un'altra espressione. In simboli, la valutazione lazy verrà indicata con il pedice $\mathrm{L}$.
    \end{frameddefn}
    ...
    \begin{framedobs}{Operazione $\mathrm{not}_\lambda$}
        Si consideri l'algebra dei booleani di Church; su di essa, è possibile definire l'operazione logica di negazione come segue: $$\mathrm{not}_\lambda := \fn{z}{(\fn{x}\fn{y}{zyx})} \iff \fn{zxy}{zyx}$$ poiché, dato un $z \in B_\lambda$, tale funzione passa a $z$ le variabili $x$ ed $y$ scambiate di posto. Si noti che l'operazione è simile all'operazione $\mathrm{ite}_\lambda$ definita nell'\cref{ite}, poiché è possibile interpretarla come l'inverso dell'operazione \tit{if-then-else}.
    \end{framedobs}
    ...
    \begin{frameddefn}{Lambda calcolo}
        Il \tbf{lambda calcolo} è un sistema formale atto ad analizzare formalmente le funzioni e le loro applicazioni. La grammatica del lambda calcolo è la seguente $$M,N ::= x \ | \ \lambda x . M \ \mid \ MN$$ dove $\lambda x.M$ indica una funzione della forma $\fn{x}{M}$, e prende il nome di \tbf{lambda astrazione}. Le espressioni del lambda calcolo sono dette \tbf{lambda espressioni}.
    \end{frameddefn}
    ...
    \begin{example}[Sostituzioni]
        I seguenti sono esempi di sostitutioni in espressioni:
    ...
    \begin{frameddefn}{Store}
        TODO SPOSTA GLI STORE DELL'ESEMPIO 3.1.1.2 QUA
    \end{frameddefn}
    ...
    \begin{frameddefn}{Clausola $;$}
        Sia $G$ una grammatica composta da programmi, e sia $G_p$ l'insieme dei suoi programmi; allora, è possibile definire su $G$ una funzione \curlyquotes{;}, come segue: $$\func{\textrm{\curlyquotes{;}}}{G_p \times G_p}{G_p}$$ e verrà utilizzata attraverso la sintassi $$\ttt{<program>}_1;\ttt{<program>}_2$$ che sta ad indicare che verrà prima eseguito $\ttt{<program>}_1$, e successivamente $\ttt{<program>}_2$.
    \end{frameddefn}

    \begin{frameddefn}{Clausola \tit{ite}}
    ...
    \begin{example}[Grammatica $Imp$]
        \label{imp}
        Si consideri la grammatica $Exp$ definita all'interno dell'\cref{exp} (si noti che non si tratta di $Exp$ estesa); è possibile estenderla come segue: $$M,N ::= k \ | \ true \ | \ false \ | \ x \ | \ M \ \ttt + \ N \ | \ M \ \ttt * \ N \ | \ M \ \ttt < \ N$$ dove $true$ e $false$ sono valori booleani di verità. Allora, sia $Imp$ la seguente grammatica: $$p,q ::= skip \ | \ p;q \ | \ \ite{M}{p}{q} \ | \ \while{M}{p} \ | \ \varin{x}{M}{p} \ | \ x:=M $$ L'insieme degli ambienti di $Imp$ viene definito come segue $$\mathrm{Env} := \{f \mid f : \mathrm{Var} \xrightarrow{fin} \mathrm{Loc} \}$$ dove $\mathrm{Loc}$ è un insieme di locazioni di memoria; inoltre, ad $Imp$ viene fornito il seguente insieme $$\mathrm{Store} := \{f \mid f : \mathrm{Loc} \xrightarrow{fin} \mathrm{Val} \}$$ ed infine, vengono definiti due tipi di giudizi operazionali per $Imp$, come segue: $$\centeredsoe{\leadsto_M \ \subseteq \mathrm{Env} \times Exp \times \mathrm{Store} \times \mathrm{Val} \\ \leadsto_p \ \subseteq \mathrm{Env} \times Imp \times \mathrm{Store} \times \mathrm{Store}}$$ poiché i giudizi operazionali delle espressioni --- della prima relazione --- sono in grado di restituire valori, ma non di cambiare la memoria, mentre i giudizi operazionali dei programmi --- della seconda relazione --- non restituiscono valori, ma alterano lo stato della memoria (si noti che verrà utilizzato il simbolo $\leadsto$ all'interno delle valutazioni di entrambe i giudizi). Tali giudizi operazionali verranno indicati col seguente simbolismo $$\centeredsoe{\opjudstore{E}{M}{S}{v} \\ \opjudstore{E}{p}{S}{S}}$$
    \end{example}

    \begin{example}[Semantica operazionale di $Imp$]
        Sia $Imp$ la grammatica definita all'interno dell'\cref{imp}, e siano $E \in \mathrm{Env}$ e $S \in \mathrm{Store}$; allora, su di essa si definiscono le seguenti semantiche:
        $$[const] \ \opjudstore{E}{k}{S}{S}$$
        $$\forall x \in \mathrm{Var} \quad \exists v \in \mathrm{Var} \mid S(E(x)) = v \implies [vars] \ \opjudstore{E}{x}{S}{v}$$
        \centeredeq{0.9}{$\forall M,N \in Exp, v \in \mathrm{Val} \quad \exists v_1, v_2 \in \mathrm{Val} \mid v = v_1 + v_2 \implies [plus] \ \dfrac{\opjudstore{E}{M}{S}{v_1} \quad \opjudstore{E}{N}{S}{v_2}}{\opjudstore{E}{M \ \ttt + \ N}{S}{v}}$}
        \centeredeq{0.9}{$\forall M,N \in Exp, v \in \mathrm{Val} \quad \exists v_1, v_2 \in \mathrm{Val} \mid v = v_1 \cdot v_2 \implies [times] \ \dfrac{\opjudstore{E}{M}{S}{v_1} \quad \opjudstore{E}{N}{S}{v_2}}{\opjudstore{E}{M \ \ttt * \ N}{S}{v}}$}
        $$\forall M, N \in Exp, v_1, v_2 \in \mathrm{Store} \quad v_1 < v_2 \implies [lt_1] \ \dfrac{\opjudstore{E}{M}{S}{v_1} \quad \opjudstore{E}{N}{S}{v_2}}{\opjudstore{E}{M \ \ttt < \ N}{S}{true}}$$
        $$\forall M, N \in Exp, v_1, v_2 \in \mathrm{Store} \quad v_1 \ge v_2 \implies [lt_2] \ \dfrac{\opjudstore{E}{M}{S}{v_1} \quad \opjudstore{E}{N}{S}{v_2}}{\opjudstore{E}{M \ \ttt < \ N}{S}{false}}$$
        $$[skip] \ \opjudstore{E}{skip}{S}{S}$$
        $$\forall p, q \in Imp, S_1, S_2 \in \mathrm{Store} \quad [seq] \ \dfrac{\opjudstore{E}{p}{ S}{S_1} \quad \opjudstore{E}{q}{ S_1}{S_2}}{\opjudstore{E}{p;q}{S}{S_2}}$$
        $$\forall p, q \in Imp, S_1 \in \mathrm{Store} \quad [ite_1] \ \dfrac{\opjudstore{E}{M}{ S}{true} \quad \opjudstore{E}{p}{S}{S_1}}{\opjudstore{E}{\ite{M}{p}{q}}{S}{S_1}}$$
        $$\forall p, q \in Imp, S_2 \in \mathrm{Store} \quad [ite_2] \ \dfrac{\opjudstore{E}{M}{ S}{false} \quad \opjudstore{E}{p}{S}{S_2}}{\opjudstore{E}{\ite{M}{p}{q}}{S}{S_2}}$$
        \centeredeq{0.9}{$\forall M \in Exp, S_1, S_2 \in \mathrm{Store} \quad [while_1] \ \dfrac{\opjudstore{E}{M}{S}{true} \quad \opjudstore{E}{p}{S}{S_1} \quad \opjudstore{E}{\while{M}{p}}{ S_1}{S_2}}{\opjudstore{E}{\while{M}{p}}{ S}{S_2}}$}
        $$\forall M \in Exp \quad [while_2] \ \dfrac{\opjudstore{E}{M}{S}{false}}{\opjudstore{E}{\while{M}{p}}{S}{S}}$$
        $$\forall x \in \mathrm{Var}, M \in Exp, p \in Imp, S_1 \in \mathrm{Store}, v \in \mathrm{Val} \quad \exists l \in \mathrm{Loc} \mid l \notin \mathrm{codom}(E) \implies$$
        $$\implies [var] \ \dfrac{\opjudstore{E}{M}{S}{v} \quad \opjudstore{E\{(x}{l)\}}{p,S\{(l,v)\}}{S_1}}{\opjudstore{E}{\varin{x}{M}{p}}{S}{S_1}}$$
        $$\forall x \in \mathrm{Var}, M \in Exp \quad \exists l \in \mathrm{Loc} \mid E(x) = l \implies [assign] \ \dfrac{\opjudstore{E}{M}{S}{v}}{\opjudstore{E}{x := M}{ S}{S\{(l, v)\}}}$$
    \end{example}

    \begin{example}[Grammatica $All$]
        Sia $All$ la grammatica composta dalle seguenti grammatiche:

        $$k ::= \ttt 0 \ | \ \ttt 1 \ | \ \ldots \ | \ true \ | \ false$$
        $$V ::= x \ | \ x[M]$$
        $$M, N ::= k \ | \ V \ | \ M \ \ttt + \ N \ | \ M \ \ttt < \ N$$
        $$p,q ::= skip \ | \ p;q \ | \ \ite{M}{p}{q} \ | \ \while{M}{p} \ | \ \varin{x}{M}{p} \ |$$
        $$| \ \arrin{x}{[M_0, \ldots, M_n]}{p} \ | \ V := M \ | \ V:= M \ | \ \procin{y}{x}{p}{x} \ | \ \call{y}{M}$$
