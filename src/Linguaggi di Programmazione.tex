...
\begin{document}
    ...
    \begin{frameddefn}{Punto fisso}
        Data una funzione $f$, un elemento $x$ è detto \tbf{punto fisso di $f$} se e solo se $f(x) = x$.
    \end{frameddefn}
    ...
    \subsection{Combinatore di Kleene}
    ...
    \begin{example}[Semantica assiomatica di $Imp$]
        \label{imp bool}
        TODO $$\leftsoe{M,N ::= k \ | \ x \ | \ M \ \ttt + \ N \ | \ M \ \ttt * \ N \\ A,B ::= true \ | \ false \ | \ A \supset B \ | \ M <  N \ | \ M = N \\ p,q ::= skip \ | \ p;q \ | \ x:=M \ | \ \ite{B}{p}{q} \ | \ \while{B}{p}}$$
    \end{example}

    \begin{example}[Significati assiomatici di $Imp$]
        Si consideri la grammatica definita all'interno dell'\cref{imp bool}; su di essa, si definiscono i seguenti \tit{significati assiomatici}, anche dette \tit{regole di inferenza speciali}:
        ...
        \begin{itemize}
            ...
            \item \tbf{if-then-else}: $$\dfrac{\hoare{A \land C}{p}{B} \quad \hoare{A \land \lnot C}{q}{B}}{\hoare{A}{\ite{C}{p}{q}}{B}}$$ poiché entrambe le triple di Hoare devono avere $B$ come postcondizione, ma TODO; si noti che anche la seguente regola di inferenza è una valida alternativa $$\dfrac{\hoare{A}{p}{B} \quad \hoare{A}{q}{B}}{\hoare{A}{\ite{C}{p}{q}}{B}}$$ ma questa risulta essere una regola \tit{più debole} della precedente, poiché le triple $\hoare{A}{p}{B}$ e $\hoare{A}{q}{B}$ sono \tit{più forti}, in quanto hanno meno precondizioni da verificare, ma questo le rende anche più difficili da dimostrare;
            \item TODO
        \end{itemize}
    \end{example}
