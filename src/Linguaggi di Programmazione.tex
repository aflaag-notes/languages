...
\begin{document}
    ...
    \begin{frameddefn}[label={store},breakable]{Memoria}
        Sia $G$ una grammatica composta da programmi, e siano $G_p$ e $G_M$ rispettivamente l'insieme dei suoi programmi e delle sue espressioni; per simulare la \tbf{memoria}, all'interno del paradigma imperativo verranno utilizzati ambienti definiti come segue: $$\mathrm{Env} := \{f \mid f : \mathrm{Var} \xrightarrow{fin} \mathrm{Loc} \}$$ dove $\mathrm{Loc}$ è un insieme di locazioni di memoria; inoltre, si definisce il seguente insieme $$\mathrm{Store} := \{f \mid f : \mathrm{Loc} \xrightarrow{fin} \mathrm{Val} \}$$ dunque, le funzioni $E \in \mathrm{Env}$ associano le variabili ad una locazione in memoria, mentre le funzioni $S \in \mathrm{Store}$ associano le locazioni ai valori, simulando quindi la struttura dei \tit{puntatori}, i quali caratterizzano i linguaggi imperativi.

        Infine, per effettuare le valutazioni, vengono definiti i due seguenti tipi di giudizi operazionali: $$\centeredsoe{\stackrel{M}{\leadsto}\ \subseteq \mathrm{Env} \times G_M \times \mathrm{Store} \times \mathrm{Val} \\ \stackrel{p}{\leadsto} \ \subseteq \mathrm{Env} \times G_p \times \mathrm{Store} \times \mathrm{Store}}$$ poiché i giudizi operazionali delle espressioni --- della prima relazione --- sono in grado di restituire valori, ma non di cambiare la memoria, mentre i giudizi operazionali dei programmi --- della seconda relazione --- non restituiscono valori, ma alterano lo stato della memoria. Per brevità, verrà utilizzato il simbolo $\leadsto$ all'interno delle valutazioni di entrambe i giudizi.

         Tali giudizi operazionali verranno indicati col seguente simbolismo $$\centeredsoe{\opjudstore{E}{M}{S}{v} \\ \opjudstore{E}{p}{S}{S}}$$
    \end{frameddefn}
    ...
    \begin{example}[Grammatica $All$]
        ...
        $$| \ \arrin{x}{[M_0, \ldots, M_n]}{p} \ | \ V := M \ | \ \procin{y}{x}{p}{x} \ | \ \call{y}{M}$$

        TODO
    \end{example}

    \begin{example}
        Sia \cref{all} la grammatica definita all'interno dell'\cref{all}, e siano $E \in \mathrm{Env}$ e $S \in \mathrm{Store}$; allora, in aggiunta alle semantiche descritte nell'\cref{imp sem}, si definiscono le seguenti:

        $$\forall x \in \mathrm{Var} \quad E(x) = l \implies \dfrac{}{\opjudstore[V]{E}{x}{S}{l}}$$
    \end{example}
