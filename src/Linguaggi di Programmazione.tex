...
\begin{document}
    ...
    \begin{frameddefn}{Grammatica}
        Una \tbf{grammatica} è un insieme di regole che definiscono come poter manipolare un insieme di stringhe, ed agiscono su elementi sintattici detti \tbf{termini}.
    \end{frameddefn}
    ...
    \begin{frameddefn}{Forma di Backus-Naur (BNF)}
        ...
        \begin{itemize}
            \item $\ttt{<symbol>}$ è una variabile \tit{non terminale}, ovvero, può essere sostituita con regole definite dalla grammatica;
            \item il simbolo $::=$ indica che ciò che è posto alla sua sinistra deve essere sostituito con ciò che è alla sua destra;
            \item $\ttt{\_expression\_}$ è un espressione che verrà usata per rimpiazzare le variabili non terminali attraverso le regole definite dalla grammatica.
        \end{itemize}
    \end{frameddefn}

    \begin{example}[Grammatica Exp]
        \label{exp}
        Si $Exp$ la seguente grammatica: $$M, N ::= \ttt 0 \ | \ \ttt 1 \ | \ldots \ | \ M + N \ | \ M * N$$ essa definisce le regole per utilizzare i numeri in $\N$, ammettendo inoltre le operazioni \und{sintattiche} di $+$ e $*$.
    \end{example}
    ...
    \begin{example}[Linguaggio di Exp]
        Si $Exp$ la grammatica definita nell'\cref{exp}; in essa, prendendo ad esempio le stringhe $\ttt{"4"}$ e $\ttt{"23"}$, si può ottenere la stringa $\ttt{"4 + 23"}$.
    \end{example}
