...
\begin{document}
    ...
    \begin{frameddefn}{Forma di Backus-Naur (BNF)}
        La \tbf{forma di Backus-Naur} (\tit{Backus-Naur Form}) è una notazione utilizzata per descrivere la sintassi di grammatiche, ed è definita come segue: $$\ttt{<symbol>}, \ldots, \ttt{<symbol>} \ ::= \ \ttt{\_expression\_} \smid \ldots \smid \ttt{\_expression\_}$$ dove
        \begin{itemize}
    ...
    \begin{frameddefn}[label={exp}]{Grammatica $Exp$}
        Sia $Exp$ la seguente grammatica: $$M, N \ ::= \ \ttt 0 \smid \ttt 1 \smid \ldots \smid x \smid M \ \ttt + \ N \smid M \ \ttt * \ N$$ essa definisce le regole per utilizzare i numeri in $\N$, ammettendo inoltre le operazioni \und{sintattiche} di $\ttt +$ e $\ttt *$.
    \end{frameddefn}
    ...
    \begin{frameddefn}[label={exp2}]{Estensione di $Exp$}
        Sia $Exp$ la seguente estensione della grammatica presente all'interno della \cref{exp}: $$M, N \ ::= \ k \smid x \smid M \ \ttt + \ N \smid M \ \ttt * \ N \smid \letin{x}{M}{N}$$
    ...
    \begin{frameddefn}[label={fun}]{Grammatica $Fun$}
        Sia $Fun$ la seguente estensione della grammatica $Exp$, definita all'interno della \cref{exp2}: $$M,N \ ::= \ k \smid x \smid M \ \ttt + \ N \smid M \ \ttt * \ N \smid \letin{x}{M}{N} \smid \fn{x}{M} \smid MN$$
    \end{frameddefn}
    ...
    \begin{frameddefn}[label={hoare logic}]{Logica di Hoare}
        Si definisce \tbf{logica di Hoare} la seguente grammatica: $$\leftsoe{M,N \ ::= \ k \smid x \smid M \ \ttt + \ N \smid M \ \ttt * \ N \\ A,B \ ::= \ true \smid false \smid A \supset B \smid M <  N \smid M = N \\ p,q \ ::= \ skip \smid p;q \smid x:=M \smid \ite{B}{p}{q} \smid \while{B}{p}}$$ dove la prima grammatica comprende le espressioni, la seconda le espressioni booleane, e la terza i programmi.
    \end{frameddefn}
    ...
    \begin{frameddefn}{Lambda calcolo polimorfo}
        ...
        \begin{itemize}
            \item una grammatica per i \tit{tipi}, che verrà indicata con $Types$; si noti che il simbolismo $X$ indica che la grammatica include le variabili di tipo, il cui insieme verrà indicato con $TypeVar$;
    ...
    \begin{framedprop}[label={regole system F}]{Regole di inferenza di \tit{System F}}
        ...
        \begin{itemize}
            \item \tbf{generalizzazioni}: \centeredeq{0.9}{$\forall M \in Terms, A \in Types \quad \exists X \in TypeVar \mid X \notin \mathrm{free}(\Gamma) \implies [gen] \ \dfrac{\typejud{\Gamma}{M}{A}}{\typejud{\Gamma}{\Lambda X.M}{\forall X.A}}$}
            \item \tbf{specializzazioni}: $$\forall M \in Terms, A, B \in Types, X \in TypeVar \quad [spec] \ \dfrac{\typejud{\Gamma}{M}{\forall X.A}}{\typejud{\Gamma}{MB}{A[B/X]}}$$
        \end{itemize}
    \end{framedprop}
    ...
    \begin{frameddefn}{Istanza generica}
        ...
        \begin{itemize}
            \item $\exists \tau_1, \ldots, \tau_n \mid \tau' = \tau[\{\tau\}_i/\{X\}_i]$ dove con $[\{\tau\}_i/\{X\}_i]$ si indica una sostituzione simultanea di ogni $\tau_i$ con ogni $X_i$
            \item $\nexists j \in [1, m] \mid Y_j \in \mathrm{free}(\sigma)$
        \end{itemize}
    \end{frameddefn}

    \begin{example}[Istanze generiche]
        Dato il tipo $$\sigma := \forall X_1, X_2.X_1 \to X_2$$ si verifica che il tipo $\sigma'$, definito come $$\sigma' := \forall Z. (\ttt{int} \to Z) \to \ttt{bool}$$ è un'istanza generica di $\sigma$, poiché $$\soe{l}{\tau_1 := \ttt{int} \to Z \\ \tau_2 := \ttt{bool}} \implies \exists \tau_1, \tau_2 \mid \tau' := (\ttt{int} \to Z) \to \ttt{bool} = (X_1 \to X_2)[\{\tau\}_i/\{X\}_i]$$ ed inoltre $Z \notin \mathrm{free}(\sigma)$.
    \end{example}
    ...
    \begin{nonexample}[Istanze generiche]
        Dato il tipo $$\sigma := X \to Y$$ si verifica che il tipo $\sigma'$, definito come $$\sigma' := \ttt{int} \to Y$$ \tit{non} è un'istanza generica di $\sigma$, poiché $X \in \mathrm{free}(\sigma)$.
    \end{nonexample}
    ...
    \begin{framedobs}{Istanze generiche reciproche}
    ...
    \begin{frameddefn}{Grammatica $Fun_\tau$}
        ...
        \begin{itemize}
            \item due grammatiche per i \tit{tipi}, che verranno indicate genericamente con $Types$; gli elementi della grammatica prendono il nome di \tit{tipi primitivi}, mentre quelli della seconda prendono il nome di \tit{schemi di tipo};
            \item una grammatica per i \tit{termini}, che verrà indicata con $Terms$.
        \end{itemize}
    ...
    \begin{framedprop}[breakable]{Regole di inferenza di $Fun_\tau$}
        ...
        \begin{itemize}
            \item \tbf{generalizzazioni}: \centeredeq{0.9}{$\forall M \in Terms, \sigma \in Types \quad \exists X \in TypeVar \mid X \notin \mathrm{free}(\Gamma) \implies [gen] \ \dfrac{\typejud{\Gamma}{M}{\sigma}}{\typejud{\Gamma}{M}{\forall X.\sigma}}$}
    ...
    \begin{example}[Derivazione di tipi]
        Si consideri l'espressione $$\letin{x}{(\fn{y}{y})}{x \ (\fn{z}{z}) \ (x \ 5)}$$ è possibile derivarne il tipo attraverso il seguente albero di derivazione: \centeredeq{0.9}{$\dfrac{\dfrac{\dfrac{\typejud{y:Y}{y}{Y}}{\typejud{\varnothing}{\fn{y}{y}}{Y \to Y}}}{\typejud{\varnothing}{\fn{y}{y}}{\forall Y.Y \to Y}} \quad \dfrac{\dfrac{\typejud{\Gamma}{x}{\forall Y . Y \to Y}}{\typejud{\Gamma}{x}{\ttt{int} \to \ttt{int}}} \quad \dfrac{\dfrac{\dfrac{\dfrac{\typejud{\Gamma, z:Z}{z}{Z}}{\typejud{\Gamma}{\fn{z}{z}}{Z \to Z}}}{\typejud{\Gamma}{\fn{z}{z}}{\forall Z.Z \to Z}}}{\typejud{\Gamma}{\fn{z}{z}}{\ttt{int} \to \ttt{int}}} \quad \dfrac{\dfrac{\typejud{\Gamma}{x}{\forall Y.Y \to Y}}{\typejud{\Gamma}{x}{\ttt{int} \to \ttt{int}}} \quad \typejud{\Gamma}{5}{\ttt{int}}}{\typejud{\Gamma}{x \ 5}{\ttt{int}}}}{\typejud{\Gamma}{(\fn{z}{z}) \ (x \ 5)}{\ttt{int}}}}{\typejud{x:\forall Y. Y \to Y}{x \ (\fn{z}{z}) \ (x \ 5)}{\ttt{int}}}}{\typejud{\varnothing}{\letin{x}{(\fn{y}{y})}{x \ (\fn{z}{z}) \ (x \ 5)}}{\ttt{int}}}$}
    \end{example}
