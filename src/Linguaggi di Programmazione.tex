...
\begin{document}
    ...
    \begin{example}[Grammatica $Fun$]
        \label{fun}
        Sia $Fun$ la seguente estensione della grammatica $Exp$, definita all'interno dell'\cref{exp2}: $$M,N ::= k \ | \ x \ | \ M \ \ttt + \ N \ | \ M \ \ttt * \ N \ | \ \letin{x}{M}{N} \ | \ \fn{x}{M} \ | \ MN$$ Inoltre, l'insieme dei valori assegnabili ai termini di $Fun$ verrà esteso come segue $$\mathrm{Val} := \{\ttt 0, \ttt 1, \ldots \} \cup (\mathrm{Var} \times Fun \times \mathrm{Env})$$
    \end{example}
    ...
    \begin{framedobs}{$Fun_\mathrm{ED}$}
        ...
        \begin{itemize}
            \item l'insieme degli ambienti di $Fun$ viene definito come segue: $$\mathrm{Env} := \{f \mid f : \mathrm{Var} \xrightarrow{fin} \mathrm{Val}\}$$ (si noti che $\mathrm{Val}$ è stato esteso nell'\cref{fun});
    ...
    \begin{framedlem}{$Fun_\mathrm{LS}$ e $Fun_\mathrm{LD}$}
