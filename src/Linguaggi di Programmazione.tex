...
\begin{document}
    ...
    \begin{frameddefn}{Forma di Backus-Naur (BNF)}
        ...
        \begin{itemize}
            \item $\ttt{<symbol>}$ è una variabile \tit{non terminale}, ovvero, può essere sostituita con regole definite dalla grammatica (le regole possono dunque essere utilizzate ricorsivamente);
    ...
    \begin{example}[Grammatica $Exp$]
        \label{exp}
        Sia $Exp$ la seguente grammatica: $$M, N ::= \ttt 0 \ | \ \ttt 1 \ | \ldots \ | \ M \ \ttt + \ N \ | \ M \ \ttt * \ N$$ essa definisce le regole per utilizzare i numeri in $\N$, ammettendo inoltre le operazioni \und{sintattiche} di $\ttt +$ e $\ttt *$.
    \end{example}
