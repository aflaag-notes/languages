...
\begin{document}
    ...
    \begin{example}[Concatenazioni di ambienti]
        Sia $Exp$ la grammatica descritta all'interno della \cref{exp2}, e siano $$\centeredsoe{E_1 := \{(z, 3), (y, 9)\} \\ E_2 := \{(z, 4)\}}$$ due suoi ambienti; allora si ha che $$E_1E_2 := \{(z, 4), (y, 9)\}$$
    \end{example}
    ...
    \begin{framedobs}{Operatore $\rho$}
        Si noti che, per definizione dell'operatore $\rho$, si verifica che $$\rho : \soe{l}{(\rhofun{\mathrm{zero}}{h}) \ 0 = a \\ (\rhofun{\mathrm{zero}}{h}) \ (\succfn \ n) = h \ ((\rhofun{\mathrm{zero}}{h}) \ n)}$$ espresso attravreso i termini delle funzioni definite all'interno del \cref{weak term ric}.
    \end{framedobs}
    ...
    \begin{framedobs}[label={rec obs}]{Operatore $rec$}
        Si noti che, per definizione dell'operatore $rec$, si verifica che $$rec: \soe{l}{(\recfun{\mathrm{zero}}{h}) \ 0 = a \\ (\recfun{\mathrm{zero}}{h}) \ (\succfn \ n) = h \ ((\recfun{\mathrm{zero}}{h}) \ n) \ n}$$ espresso attraverso i termini delle funzioni definite all'interno del \cref{func ric prim}.
    \end{framedobs}
    ...
    \begin{example}[Operatore $rec$ in $Fun_\rho$]
        Sia $\funcmap{\mathrm{plus}}{\N \times \N}{\N}{(x,y)}{x + y}$ la funzione che somma $x$ ad $y$, definita ricorsivamente --- attraverso le espressioni della grammatica $Fun_\rho$ definita nella \cref{fun rho} --- come segue: $$\mathrm{plus} : \soe{l}{\mathrm{plus} \ 0 \ y = y \\ \mathrm{plus} \ (\succfn \ n) \ y = \succfn \ (\mathrm{plus} \ n \ y)}$$ si noti dunque che è possibile esprimerla attraverso l'operatore $rec$ come segue: $$\mathrm{plus} \equiv \fn{xy}{\recfun{y}{(\fn{wz}{\succfn \ w})} \ x}$$ poiché $$\mathrm{plus} \ 0 \ y \equiv \recfun{y}{(\fn{wz}{\succfn \ w)}} \ 0 = y$$ (si noti l'\cref{rec obs}), ed inoltre $$\centeredsoe{\mathrm{plus} \ (\succfn \ n) \ y \equiv \recfun{y}{(\fn{wz}{\succfn \ w}) \ (\succfn \ n)} \equiv \\ \equiv (\fn{wz}{\succfn \ w}) \ (\recfun{y}{(\fn{wz}{\succfn \ w})} \ n ) \ n \equiv  \\ \equiv (\fn{wz}{\succfn \ n}) \ (\mathrm{plus} \ n \ y ) \ n  \longrightarrow \succfn \ (\mathrm{plus} \ n \ y)}$$
    \end{example}

    % \begin{example}[Operatore $rec$ in $Fun_\rho$]
    %     Sia $\funcmap{p_2}{\N}{\N}{n}{n + 2}$ la funzione che somma 2 all'input fornito, definita ricorsivamente --- attraverso le espressioni della grammatica $Fun_\rho$ definita nella \cref{fun rho} --- come segue: $$p_2 : \soe{l}{p_2 \ 0 = 2 \\ p_2 \ (\succfn \ n) = \succfn \  (p_2 \ n)}$$ si noti dunque che è possibile esprimerla attraverso l'operatore $rec$, considerando la funzione $\funcmap{\mathrm{due}}{\1}{\N}{x}{2}$ --- come segue: $$p_2 \equiv \recfun{\mathrm{due}}{(\fn{xy}{\succfn \ (\succfn \ x))}}$$ poiché $$p_2 \ 0 \equiv \recfun{\mathrm{due}}{(\fn{xy}{\succfn \ (\succfn \ x))}}\ 0 = 2$$ (si noti l'\cref{rec obs}), ed inoltre $$\centeredsoe{p_2 \ (\succfn \ n) \equiv \recfun{\mathrm{due}}{(\fn{xy}{\succfn \ (\succfn \ x))}}\ (\succfn \ n) \equiv \\ \equiv (\fn{xy}{\succfn \ (\succfn \ x))} \ ((\recfun{\mathrm{due}}{(\fn{xy}{\succfn \ (\succfn \ x))}}) \ n) \ n \equiv \\ \equiv (\fn{xy}{\succfn \ (\succfn \ x))} \ (p_2 \ (\succfn \ n)) \ n \longrightarrow (\fn{y}{\succfn \ (\succfn \ (p_2 \ (\succfn \ n)))}) \ n}$$
    % \end{example}
